...
const EmojiPickerComponent = ({ onEmojiSelect, onClose }) => {
  ...
  return (
    <div style={{
      display: "grid", gridTemplateColumns: "repeat(8, 1fr)", gap: "8px", padding: "15px",
      background: "#1f2937", borderRadius: "16px", width: "350px", maxWidth: "90vw",
      maxHeight: "400px", overflowY: "auto", border: "1px solid #4f46e5",
      boxShadow: "0 10px 25px rgba(0,0,0,0.3)", position: "absolute", bottom: "70px",
      right: "10px", zIndex: 1000
    }}>
      {emojis.map((emoji, index) => (
        <button key={index} onClick={() => { onEmojiSelect(emoji); onClose(); }}
          style={{ fontSize: "28px", background: "transparent", border: "none", cursor: "pointer",
            padding: "8px", borderRadius: "8px", transition: "all 0.2s ease" }}
          onMouseEnter={(e) => { e.target.style.transform = "scale(1.2)"; e.target.style.background = "#4f46e520"; }}
...
const ChatArea = ({ selectedUser, Userprofile, id, token, prev_msg, uid, onBack }) => {
  ...
  // Hardware back button: closes chat and shows sidebar
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    socketRef.current.on("receive_message", (msg) => {
      setMessages((prev) => [...prev, {
        text: msg.text, isOwn: false, seen: false, timestamp: new Date()
      }]);
      setindicator("");
    });
  ...
  // ✅ FIXED: Load previous messages and sort by timestamp
  useEffect(() => {
    if (uid && prev_msg && Array.isArray(prev_msg)) {
      const formatted = prev_msg
        .map((msg) => ({
          text: msg.text,
  ...
  const getResponsiveStyles = () => {
    ...
    if (width <= 480) return { headerHeight: "55px", avatarSize: "35px", fontSize: "14px", bubblePadding: "8px 12px", inputPadding: "10px" };
    if (width <= 768) return { headerHeight: "60px", avatarSize: "40px", fontSize: "15px", bubblePadding: "10px 14px", inputPadding: "12px" };
    return { headerHeight: "65px", avatarSize: "40px", fontSize: "16px", bubblePadding: "10px 14px", inputPadding: "12px" };
  };
  ...
  const styles = {
    wrapper: {
      position: "fixed", top: `${dragOffset}px`, left: 0, right: 0, bottom: 0,
      display: "flex", flexDirection: "column", background: "linear-gradient(180deg, #1f2937, #111827)",
      zIndex: 1000, transition: isDragging.current ? "none" : "top 0.2s ease",
    ...
    header: {
      height: responsive.headerHeight, display: "flex", alignItems: "center", justifyContent: "space-between",
      padding: isMobile ? "8px 12px" : "12px 20px", background: "rgba(255,255,255,0.05)",
      backdropFilter: "blur(10px)", borderBottom: "1px solid rgba(255,255,255,0.08)", flexShrink: 0,
    },
    ...
    return (
      <div style={styles.wrapper}>
        <div style={{ flex: 1, display: "flex", justifyContent: "center", alignItems: "center", textAlign: "center", padding: "20px" }}>
          <div><div style={{ fontSize: "80px", marginBottom: "20px" }}>💬</div><h3 style={{ color: "#fff" }}>Welcome to ChatApp</h3><p style={{ color: "#9ca3af" }}>Select a friend to start messaging</p></div>
        </div>
      </div>
    );
  }
      ...
      <div style={styles.header}>
        <div style={styles.headerLeft}>
          ...
          <div style={styles.avatar}>
            <img src={Userprofile || "https://via.placeholder.com/40"} alt="Profile" style={styles.avatarImage} onError={(e) => (e.target.src = "https://via.placeholder.com/40")} />
          </div>
      ...
      <div style={styles.chatBox}>
        {messages.length === 0 ? (
          <div style={{ textAlign: "center", color: "#6b7280", marginTop: "40px" }}>
            <span style={{ fontSize: "40px" }}>💬</span><p>No messages yet</p><small>Say hello to {selectedUser}</small>
          </div>
        ) : (
          messages.map((m, i) => (
            <div key={i} style={{ display: "flex", justifyContent: m.isOwn ? "flex-end" : "flex-start", marginBottom: "8px" }}>
              <div style={{ maxWidth: isMobile ? "80%" : "70%" }}>
                <div style={{ ...styles.bubble, background: m.isOwn ? "#4f46e5" : "#1f2937", borderBottomRightRadius: m.isOwn ? "4px" : "18px", borderBottomLeftRadius: m.isOwn ? "18px" : "4px" }}>
                  {m.text}
                </div>
