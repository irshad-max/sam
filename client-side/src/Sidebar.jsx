...
const Sidebar = ({ token, user_id, getmsg, onUserSelect, isMobile = false, currentUserId }) => {
  ...
  const fetchRequests = async () => {
    const res = await axios.post("/request-show", {}, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const sendRequest = async (receiverId) => {
    try {
      await axios.post("/request", { receiver: receiverId }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const confirmRequest = async (requestId) => {
    try {
      await axios.post(`/accept-request/${requestId}`, {}, { headers: { Authorization: `Bearer ${token}` } });
